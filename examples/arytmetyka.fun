...
type Num = Infinity of float or
           Number of float
           NaN
type Value = Interval of float * float or
             Real

let interval_from_to from to =
    Interval from to

let singleton x = 
    interval_from_to x x

let interval_precision x precision =
    let dev = x * (precision / 100) in
    interval_from_to (x + dev) (x - dev)

let max_value interval =
    match interval with
    |   Interval min max ->
            if min > max then
                Infinity 1
...
let in_interval interval value =
    match interval with
    |   Interval min max ->
            if min <= max then
            ...
            |   Interval min2 max2 ->
                    if min1 > max1 then
                        if min2 > max2 then
                            ...
                            let min = max1 + max2 in
                            let max = min1 + min2 in
                            if min > max then
                                Real
                            else
                                Interval max min
    ...
    |   Interval min1 max1 ->
            ...
            |   Interval min2 max2 ->
                    if min1 > max1 then
                        ...
                        if min2 > max2 then
                            mul interval2 interval1
                        else
                            let min = min1 * max2 in
                            let max = max1 * min2 in
                            if min < max then
                                Interval min max
                            else
                                Interval max min       
...
let subtract interval1 interval2 =
    add interval1 (mul interval2 (singleton (-1)))

let div interval1 interval2 =
    match interval1 with
    |   Interval min1 max1 ->
            match interval2 with
            |   Interval min2 max2 ->
...
# let f = mul c e
# let g = mul d e
# let h = interval_precision (-10) 50
# let i = div h e
let j = interval_from_to (-6) 5
let k = mul j j
let l = add a b
let m = mul b l
# let n = div l l
...
# let q = add n 0
# let r = subtract n n
# let s = interval_precision (-0.0001) 100
# let t = mul n s
