...
def test_wrong_syntax_greater_than() -> None:
    ...
    [(res, err)] = basic.run(" let _ = 42>", "test_wrong_order_greater_than")
    assert err is not None
    assert res is None
    assert isinstance(err, InvalidSyntaxError)

    [(res, err)] = basic.run(" let _ = 42.5>", "test_wrong_order_greater_than_float")
    assert err is not None
    assert res is None
    assert isinstance(err, InvalidSyntaxError)
...
def test_integer_greater_than() -> None:
    ...
    [(res, err)] = basic.run(" let _ = 4 > +2", "test_positive_integer_greater_than")
    assert err is None
    ...
    [(res, err)] = basic.run(
        " let _ = 4 > 4", "test_positive_integer_greater_than_equals"
    )
    assert err is None
    assert res is not None
    assert isinstance(res, LangNumber)
...
def test_float_greater_than() -> None:
    ...
    [(res, err)] = basic.run(" let _ = 4.5 > +2.3", "test_positive_float_greater_than")
    assert err is None
    assert res is not None
    assert isinstance(res, LangNumber)
...
def test_mixed_greater_than() -> None:
    ...
    [(res, err)] = basic.run(" let _ = 4.5 > 5", "test_positive_float_greater_than")
    assert err is None
    assert res is not None
    assert isinstance(res, LangNumber)
