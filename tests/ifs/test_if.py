...
def test_wrong_syntax_if() -> None:
    ...
    [(res, err)] = basic.run(
        " let _ = if 1 then 2 elif 3 then 2", "test_wrong_syntax_if"
    )
    assert err is not None
    assert res is None
    assert isinstance(err, InvalidSyntaxError)
...
def test_if() -> None:
    ...
    [(res, err)] = basic.run(
        " let _ = if 0 then 1 elif 1 then 2 else 3", "test_if_elif_1"
    )
    assert err is None
    assert res is not None
    assert isinstance(res, LangNumber)
    ...
    [(res, err)] = basic.run(
        " let _ = if 0 then 1 elif 0 then 2 else 3", "test_if_elif_2"
    )
    assert err is None
    assert res is not None
    assert isinstance(res, LangNumber)
